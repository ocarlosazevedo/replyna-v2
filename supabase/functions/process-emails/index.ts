...
async function processMessageInternal(
  ...
  if ((!cleanBody || cleanBody.trim().length < 3) && message.body_html && message.body_html.trim().length > 10) {
    console.log(`[process-emails] body_text vazio mas body_html tem ${message.body_html.length} chars, extraindo texto`);
  ...
  if (classification.category === 'suporte_humano' || classification.category === 'edicao_pedido') {
    ...
    if (knownOrderNumber) {
      ...
      // MAX_DATA_REQUESTS excedido sem número de pedido - marcar como pending_human
      await releaseCredit(user.id);
      await updateMessage(message.id, {
        status: 'pending_human',
        category: classification.category,
      ...
      console.log(`[processMessage] Message ${message.id} marked as pending_human (max data requests exceeded)`);
      return 'forwarded_human';
    }
  }
