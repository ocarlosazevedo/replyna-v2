...
/**
 * Registra que o usuário ficou sem créditos.
 * NÃO cobra automaticamente - o usuário verá o banner no dashboard
 * e pode comprar mais créditos manualmente na página de conta.
 */
export async function notifyCreditsExhausted(userId: string): Promise<{
  notified: boolean;
  error?: string;
}> {
  const supabase = getSupabaseClient();
  ...
  const { data: user } = await supabase
    .from('users')
    .select('id, email, emails_used, emails_limit, extra_emails_purchased')
    .eq('id', userId)
    .single();

  if (!user) {
    return { notified: false, error: 'Usuário não encontrado' };
  }

  // Verificar se é plano ilimitado
  if (user.emails_limit === null) {
    return { notified: false, error: 'Plano ilimitado' };
  }
